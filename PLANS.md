...
The active `Workflow v1 Execution Safety Boundary Rebaseline` preserves the Workflow as the **Human-Governed AI Engineering Control Layer** while correcting execution-safety semantics. `allowed_files` is exact legitimate candidate-modification and acceptance scope; it is not an automatic per-path OS sandbox claim. The local Runner retains bounded Git and allowed-file manifest evidence under `local_git_candidate_observation.v1`, reports current Codex `workspace-write` isolation as `unverified`, emits only candidate-snapshot eligibility tokens after valid governance and verified named evidence, and revalidates current scope/evidence in CommitApproved. Universal write absence, transient-action prevention, exhaustive Git-internals coverage, network/process isolation, and external-side-effect isolation remain non-claims. This implementation does not itself make Workflow v1 `DONE`.
...
The first three mandatory nodes remain historically `DONE`. Workflow v1 Final Closeout is in `REVIEW — Execution Safety Boundary Rebaseline under review`, and Workflow v1 final `DONE` acceptance is withheld. The current active engineering node is `Workflow v1 Execution Safety Boundary Rebaseline`, activated by the user's explicit bounded approval. Tracker #168 remains stale and pending final truth synchronization; this task does not mutate it. The correction creates no Issue #188, benchmark, startup, tray, service, MCP, repository-separation, dependency, live-Bridge, trusted-actor, allowlist expansion, provider-isolation implementation, or automatic Git/GitHub authority. No later node is activated.

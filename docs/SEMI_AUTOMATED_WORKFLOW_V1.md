...
User
→ ChatGPT
→ GitHub Issue / Comment / Task Packet
→ Codex App / Codex Web / VS Code Codex Extension / local runner
→ GitHub-visible or user-provided result
→ ChatGPT review
...
User gives direction to ChatGPT
ChatGPT classifies mode and risk
ChatGPT prepares task draft or Codex prompt
User approves if the action is medium-risk, high-risk, or GitHub write
Codex App / Codex Web / VS Code Codex Extension / GitHub performs the scoped action
Result is returned or made visible
ChatGPT reviews result
ChatGPT reports outcome, risk, adopt / reject / revise recommendation, and next action
User approves only when required
Next phase begins

...
- **CONDITIONAL:** Chat direct, Codex, Work, native Chat-to-Codex routing, formal manifests, evidence collectors, systematic debugging, manual relay fallback, a Native-vs-Bridge benchmark when a real unresolved decision requires it, and optional parallel agents.
...
Classification: **OPTIONAL / CONDITIONAL OPERATING MODE**. Parallel agents are not a mandatory Workflow v1 dependency, do not imply three-times throughput, and do not receive permanent autonomous or concurrent-write authority.
...
After canonical acceptance, current truth is distributed intentionally:
...
The repository-separation plan, old proof report, and earlier operational baseline retain historical and design value, but their embedded phase statements do not override `PLANS.md` or this closeout after acceptance.
